...
public class CartController: ControllerBase
{
    ...
    public async Task<ActionResult> AddItemsToCart([FromBody] CartWithItemsRequest cartRequest)
    {
        var result = await _cartService.AddItemToCart(cartRequest.UserId, cartRequest.Items.ProductId, cartRequest.Items.Quantity);

        return result.IsFailed ? result.ToActionResult() : Ok();
    }
    
    [HttpDelete("remove-item-from-cart")]
    public async Task<ActionResult> RemoveItemFromCart([FromBody] RemoveItemFromCartRequest cartRequest)
    {
